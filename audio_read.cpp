...
{
    // ///视频没有开始采集 不采集音频
    if ( isBegin && m_saveVideoFileThread->mVideoDataList.size() == 0 )//音频线程开始
        //需要等视频线程
        {
            myBuffer_in->readAll();
            return;
    ...
    // 关于长度 采样数 即 1024
    int count =swr_convert(swr, out_buffer , nFrameCount*OneAudioSize/4,(const uint8_t**)&/*pInData*/in_buffer,nFrameCount*OneAudioSize/4);//len 为 4096
    //每次 读取 一帧
...
void Audio_Read::setSaveVideoFileThread(SaveVideoFileThread *newSaveVideoFileThread)
{
    m_saveVideoFileThread = newSaveVideoFileThread;
}
